...
import yaml
# from shap_e.util.dist_helper import env, setup_distributed, finalize
...
class Mydataset(Dataset):
    def __init__(self, latent_code_path, view_indexs):
        self.data_list = [line.strip().split('/')[-1] for line in open('/mnt/pfs/users/liyangguang/vastcode/multiview_dataset/10class_minidata_clean.txt')]
        self.latent_code_path = latent_code_path
        self.view_indexs = view_indexs

    def __len__(self):
        return len(self.data_list)

    def __getitem__(self, i):
        uuid, caption = self.data_list[i], ''
        images = []
        # if args.random_view:
        #     view_indexes = [random.randint(1, 24) for _ in range(VIEW_NUM)]
        # else:
        #     view_indexes = VIEW_INDEXS
        
        for image_index in self.view_indexs:
        ...
        # latent = torch.load(os.path.join(self.latent_code_path, self.data_list[i]))
...
class Mydataset_val(Dataset):
    def __init__(self, latent_code_path, view_indexs):
        ...
        self.latent_code_path = '/mnt/pfs/users/yuzhipeng/shapeE/shap-e/spaceship/all_latent'
        self.view_indexs = view_indexs
...
def train(rank, args):
    ...
    my_dataset = Mydataset(args.latent_code_path, args.view_indexs)
    sampler_train = DistributedSampler(my_dataset)
    data_loader = torch.utils.data.DataLoader(my_dataset, batch_size=batch_size, num_workers=3, persistent_workers=True, sampler=sampler_train, pin_memory=True, drop_last=True)
    print('sampler_train: {}'.format(len(sampler_train)))
    print('data_loader: {}'.format(len(data_loader)))
    # data_loader = DataLoader(my_dataset, batch_size=batch_size, num_workers=0, prefetch_factor=4, shuffle=True, drop_last=True)
    # data_loader = DataLoader(
        # my_dataset, batch_size=batch_size, num_workers=3, shuffle=True, drop_last=True, )
    my_dataset_val = Mydataset_val(args.latent_code_path, args.view_indexs)
    sampler_val = DistributedSampler(my_dataset_val)
    data_loader_val = torch.utils.data.DataLoader(my_dataset_val, batch_size=batch_size, num_workers=3, persistent_workers=True, sampler=sampler_val, pin_memory=True, drop_last=True)
    print('sampler_val: {}'.format(len(sampler_val)))
    print('data_loader_val: {}'.format(len(data_loader_val)))
    # data_loader_val = DataLoader(my_dataset_val, batch_size=batch_size, num_workers=0, prefetch_factor=4, drop_last=True)
    # data_loader_val = DataLoader(
        # my_dataset_val, batch_size=batch_size, num_workers=3, drop_last=True, )

    # optimizer = optim.AdamW(model.parameters(), lr=learning_rate) #1
    ...
    lr_scheduler = optim.lr_scheduler.CosineAnnealingLR(
        optimizer, niter*len(data_loader))
    if resume_flag:
    ...
    for epoch in range(start_epoch, niter):
        ...
        for i, data in enumerate(data_loader):
            ...
            # ts = np.random.choice(np.arange(1, diffusions.betas.shape[0]), batch_size)
...
if __name__ == '__main__':
    ...
    model_group.add_argument(
        '--model_path', type=str, default='/mnt/pfs/users/yuzhipeng/shapeE/shap-e/pretrains/image_cond.pt', help='')
    model_group.add_argument(
        '--view_indexs', default=[1,2,4,5])
    args = parser.parse_args()
    # VIEW_INDEXS = [1, 2, 4, 5]
    model_config = yaml.load(open(args.model_config), Loader=yaml.FullLoader)
    print(model_config)
    # VIEW_NUM = model_config['inner']['num_views']
    # VIEW_INDEXS = [i for i in range(1, VIEW_NUM+1)]
